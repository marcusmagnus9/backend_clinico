...
export const loginUsuario = async (req, res) => {
  try {
    const userFound = await getConnection().get("Users").find({ usuario: req.body.usuario }).value();
    if (!userFound) return res.status(400).json({ message: "Usuario No Existe" });
    const matchPassword = await await bcrypt.compare(req.body.password, userFound.password);
    if (!matchPassword)
      return res.status(401).json({token: null, message: "Password Invalido",
    });
    const token = jwt.sign({ id: userFound.id }, config.SECRET, {
      expiresIn: 252000, // 7 hours
    });
    res.json({ token });
  } catch (error) {
    console.log(error);
